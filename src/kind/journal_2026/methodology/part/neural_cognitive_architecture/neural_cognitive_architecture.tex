...
\begin{document}
   ...
   \section{Anatomy of a neuron}
      ...
      \subsection{Converting evidence probability distributions to EPSP and IPSP}
         ...
         denote state neuron $i$ at abstraction level $\ell$, where
         ...
         \mathrm{word},
         \mathrm{sentence},
         \mathrm{final\ concept}
         \},
         \qquad
         d \in \{0,2\}.
         \]

         Here, $d=0$ denotes an intensity-encoding neuron, whereas $d=2$
         denotes an intensity-change-rate-encoding neuron.
         ...
         denote sensory/receptor neuron $j$, and let

         \[
         Z_j
         \]

         denote its observation.

         The probability that the state represented by neuron
         $X_{i,\ell}^{(d)}$ is present, given observation $Z_j$, is
         ...
         The corresponding signed evidence is obtained through the log-odds
         transformation
         ...
         The excitatory and inhibitory contributions are then

         \[
         \mathrm{EPSP}_{i,\ell,j}^{(d)}
         =
         \max
         \left(
         0,
         ...
         \begin{cases}
            >0,
            &
            P\left(X_{i,\ell}^{(d)}=1\mid Z_j\right)>0.5,
            \\[2mm]
